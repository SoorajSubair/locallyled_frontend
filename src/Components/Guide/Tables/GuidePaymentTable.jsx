...
import {baseUrl, guidePaidPayments, guidePendingPayments } from "../../../Utils/Urls";
import { useSelector } from "react-redux";


export default function GuidePaymentTable() {
    ...
    const [paymentType, setpaymentType] = useState('recieved')
    const { id } = useSelector(state => state.user);
    const url = id ? `${guidePaidPayments}${id}` : "";
    const [paymentUrl, setPaymentUrl] = useState(url)
  ...
  useEffect(() => {
    ...
    const representativeBodyTemplate = (rowData) => {
      const image = rowData.booking.user.image;
      const firstName = rowData.booking.user.first_name;
      const lastName = rowData.booking.user.last_name
    ...
    const handlePayment = (type) => {
      setpaymentType(type)
      if(type === 'recieved'){
          setPaymentUrl(`${guidePaidPayments}${id}`)

      }
      else if(type === 'pending'){
        setPaymentUrl(`${guidePendingPayments}${id}`)
    ...
    return (
      ...
      <div className="guide-booking-button-container">
          <button onClick={()=>handlePayment('recieved')} className={paymentType==='recieved' ? "guide-booking-button booking-active" : "guide-booking-button"}>Recieved</button>
          <button onClick={()=>handlePayment('pending')} className={paymentType==='pending' ? "guide-booking-button booking-active" : "guide-booking-button"}>Pending</button>
      </div>
      {customers===null?
      <>
      <div class="Container-1G5cP">
          ...
          <div class="TextContainer-2_LWV">
              <h2 class="Title-1gcCM">No {paymentType} payments</h2>
          </div>
        </div>
        </>
        :
        <div className="guide-schedule-card">
            <DataTable value={customers} paginator rows={5} dataKey="id" filters={filters} filterDisplay="row" loading={loading}
                ...
                <Column field="amount" header="Amount" style={{ minWidth: '12rem' }} body={(rowData) => {return `$ ${rowData.amount}`;}}/>
            </DataTable>
        </div>
        }
        </>
